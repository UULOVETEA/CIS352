...
-- Author(s): Xiaomeng Cao

import LFPas
import LFPparser
import State
import Data.List   ((\\),nub,union,sort,delete)
...
freeVars (Var x)           = [x]
freeVars (Fun x e)         = (delete x (multiFreeVars [e]))
freeVars (Let x e1 e2)     = union (delete x (multiFreeVars [e1])) (delete x (multiFreeVars [e2]))
...
subst (Fun y e)           x new
    = if x == y then (Fun y e) else (Fun z p'')
      where
        z   = fresh [y]
        p'  = subst e z (Fun y e)
        p'' = subst p' x new
subst (Let y e0 e1)       x new
    = if x == y then (Let y (subst e0 x new) e1) else (Let z (subst e0 x new) e1'')
      where 
        z    = fresh [y]
