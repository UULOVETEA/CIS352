...
-- showExpr (Or (Pred "P" [Var 0]) (And (Pred "Q" [Var 1]) (Pred "Q" [Var 2])))
--   returns: "(P[x0] + (Q[x1] & Q[x2]))"
-- showExpr (And (Pred "P" [Var 0]) (Or (Pred "Q" [Var 1]) (Pred "Q" [Var 2])))
--   returns: "(P[x0] & (Q[x1] + Q[x2]))"
...
-- showExprl (Or (Pred "P" [Var 0]) (And (Pred "Q" [Var 1]) (Pred "Q" [Var 2])))
--   returns: "P[x0] + Q[x1] & Q[x2]"
-- showExprl (And (Pred "P" [Var 0]) (Or (Pred "Q" [Var 1]) (Pred "Q" [Var 2])))
--   returns: "P[x0] & (Q[x1] + Q[x2])"
...
showExprl (And e1 e2)  
    = (parenify ((prec e1) < andPrec) (showExprl e1)) 
      ++ " & " ++
      (parenify ((prec e2) <= andPrec) (showExprl e2)) -- See (**) below
showExprl (Or e1 e2)   
    = (parenify ((prec e1) < orPrec) (showExprl e1))
      ++ " + " ++
      (parenify ((prec e2) <= orPrec) (showExprl e2))  -- See (**) below

-- (**) Note that (A & B) & C and A & (B & C) have different parse trees.
---     Since & is left-associative A & B & C == (A & B) & C.
--      Now, if we had < in place of <= above both (A & B) & C and
--      A & (B & C) would print as A & B & C.  For the purposes of this
--      exercise, I want different parse trees to have different
--      string representations, hence, we use ``<='' above instead of ``<''.
------------------------------------------------------------------------
------------------------------------------------------------------------
...
xcast = ["beppo","cheeta","curiousgeorge","koko",
